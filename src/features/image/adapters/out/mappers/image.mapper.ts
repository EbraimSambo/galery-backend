...
import { Pagination } from 'src/shared/domain/entities/pagination';
...
export class ImageMapper {
  ...
  static toDomainProps(
    raw: CreateImageProps,
    galleryInternalId: number,
  ): Image {
    return new Image(raw.name, raw.path, raw.published, galleryInternalId);
  }
  ...
  static toPaginationDto(pagination: Pagination<Image>) {
    return {
      data: pagination.data.map((image) => ImageMapper.toDto(image)),
